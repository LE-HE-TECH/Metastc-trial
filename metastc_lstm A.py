...
# MetaSTC: TKDE 论文标准配置
# ------------------------------------------------------------
# 模型: Traffic2Vec + SpatialEncoder + Gating + ClusterMemoryUnit
#       + Hypernetwork Backbone (Memory -> 预测层权重)
# 训练: 单模型端到端训练 (无 K-Means, 无三阶段)
# 配置: batch=128, epochs=50, lr=1e-3, max_batches=200,
#       patience=5, K=5, τ=0.5 (项目硬约束)
# ============================================================
...
# ==========================================
# 4. Cluster-Aware Memory Unit
# Paper Section III.B - Eq. 8-12
# τ=0.5 固定 (项目硬约束: 所有模型公平比较)
# ==========================================
...
# Paper: Memory -> 为预测层生成专属权重 (distinct parameters)
# ==========================================
class MetaBackbone(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim, mem_dim):
        ...
        # Hypernetwork: Memory Context -> 预测层权重
...
# ============================================================
# 8. 单模型端到端训练 (TKDE 标准方法)
# ============================================================
def train(model, train_loader, val_loader, config, log_prefix='A'):
    print(f"\n>>> Training MetaSTC-{log_prefix} (TKDE Standard, End-to-End)")
    ...
    for epoch in range(config['epochs']):
        ...
        val_max_batches = config['max_batches'] // 2
        with torch.no_grad():
...
# 9. 单模型测试 (TKDE 标准方法)
# ============================================================
def test(model, val_loader, max_flow, config, log_prefix='A'):
    print(f"\n>>> Testing MetaSTC-{log_prefix} (Single Model)")
    ...
    with torch.no_grad():
        for x_f, x_t, r_id, r_lvl, ln, tgt in val_loader:
            ...
            pred, _ = model(x_f, x_t, r_id, r_lvl, ln)
            all_preds.append((pred * max_flow).cpu().numpy())
            all_targets.append((tgt * max_flow).numpy())
    ...
    print("-" * 30)
    print(f"MetaSTC-{log_prefix} Results (TKDE Standard):")
...
# 10. 可视化
# ============================================================
def plot_loss(train_losses, val_losses, save_path='figure/A_loss.png'):
    """Loss 曲线"""
    ...
    plt.xlabel('Epoch')
    plt.ylabel('MSE Loss')
    plt.title('MetaSTC Training Loss (TKDE Standard)')
    plt.legend()
    plt.grid(alpha=0.3)
    plt.tight_layout()
...
def plot_prediction(y_true, y_pred, metrics, save_path='figure/A_prediction.png'):
    """预测对比图"""
    ...
    plt.title(f"MetaSTC Prediction (TKDE Standard) | MAE={metrics['mae']:.4f} "
...
def plot_cluster_alpha(model, val_loader, save_path='figure/A_cluster_alpha.png', max_batches=3):
    """聚类 α 分布热力图"""
    model.eval()
    all_alpha = []
    with torch.no_grad():
        for i, (x_f, x_t, r_id, r_lvl, ln, tgt) in enumerate(val_loader):
            ...
            x_f, x_t = x_f.to(device), x_t.to(device)
            r_id, r_lvl, ln = r_id.to(device), r_lvl.to(device), ln.to(device)
            _, alpha = model(x_f, x_t, r_id, r_lvl, ln)
    ...
    axes[0].set_title('Cluster Membership α')
...
# 11. 主入口: TKDE 标准复现
# ============================================================
def run_paper_reproduction(look_back=12):
    """TKDE 标准: 单模型端到端训练"""
    tag = f'A_L{look_back}'
    config = {
        'batch_size': 128,        # 论文配置
        ...
        'lr': 0.001,              # 学习率 (论文推荐)
        'patience': 5,            # 早停耐心值
        'split_ratio': 0.8,       # 8:2 划分 (项目硬约束)
        'pkl_path': 'data/traffic_flow/1/20230306/part-00000.pkl',
        'feature_path': 'data/link_feature.csv',
        'max_batches': 200,       # 每轮 batch 上限 (项目硬约束: 200 batches/epoch)
        'model_dir': f'param/metastc_A_L{look_back}',
    ...
    # (2) 模型 (论文参数: temporal_hidden=32, 1 层 LSTM, mem=64, Hypernetwork)
    model = MetaSTC(
    ...
    # (3) 训练 (单模型端到端)
    print(f"\n>>> MetaSTC-{tag} (TKDE Standard Configuration)")
    train_losses, val_losses = train(model, train_loader, val_loader, config, log_prefix=tag)

    # (4) 测试
    y_true, y_pred, metrics = test(model, val_loader, max_flow, config, log_prefix=tag)
...
if __name__ == "__main__":
    for L in [12, 24]:
        run_paper_reproduction(look_back=L)
